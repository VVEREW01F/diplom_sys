...
[bastion]
bastion-host ansible_host=${yandex_compute_instance.bastion.network_interface.0.nat_ip_address} ansible_ssh_user=user-data
...
ansible_ssh_common_args='-o ProxyCommand="ssh -W %h:%p user-data@${yandex_compute_instance.bastion.network_interface.0.nat_ip_address}"'
